...
export default function Home() {
  ...
  const adsToDisplay = searchedAds.length > 0 ? searchedAds 
                    : AdsFilter.length > 0 ? AdsFilter 
                    : ads;

  // Get favorite ad IDs from Redux state
  const favoriteAds = favorites?.items ? favorites.items.map(fav => fav.ad._id) : [];
  ...
  try {
    ...
    toast.error(
      ...
      `Could not ${action} favorites` // Use pre-determined action
    );
  } finally {
